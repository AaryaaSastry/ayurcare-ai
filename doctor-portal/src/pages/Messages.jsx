...
const NegotiationCard = ({ message, currentRole, onAccept, onCounter, acceptingId }) => {
  ...
  return (
    <div className="w-full max-w-[380px] rounded-[24px] border border-primary-200/90 bg-gradient-to-br from-white via-primary-50/60 to-emerald-50/60 p-4 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2.5 text-sm text-slate-700">
        <div className="rounded-2xl bg-white/85 px-3.5 py-2.5 border border-white">
      ...
      <div className="mt-4 rounded-2xl border border-primary-100 bg-white/80 px-3.5 py-2.5 text-sm">
        {isLocked ? (
      ...
      <div className="mt-3">
        {isLocked ? (
          <div className="rounded-2xl bg-primary-600 px-4 py-2.5 text-center text-sm font-bold text-white">
            Deal Confirmed
          </div>
        ) : isPending && !didAccept ? (
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-slate-100 flex-shrink-0 overflow-hidden flex items-center justify-center border border-slate-100">
                    {chat.participantProfileImage ? (
                      ...
                      <UserRound size={16} className={chat._id === activeChatId ? 'text-white/50' : 'text-slate-400'} />
                    )}
                  </div>
            ...
            {activeChat ? (
              <>
                ...
                <div className="relative flex-shrink-0 p-6 border-t border-slate-200 bg-white">
                  {isOfferOpen && (
                    <div ref={offerPanelRef} className="absolute bottom-[calc(100%+12px)] left-6 w-[380px] rounded-[2rem] border border-slate-200 bg-white p-4 shadow-2xl shadow-slate-300/40 flex flex-col gap-4 max-h-[500px] overflow-y-auto">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="space-y-1.5">
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                className="w-full pl-9 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-primary-400 outline-none transition-all"
                                placeholder="Fee"
                              />
                            </div>
                          </div>
                          <div className="space-y-1.5">
                            ...
                            <select
                              ...
                              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:border-primary-400 outline-none transition-all"
                            >
                            ...
                            {availableDays.map((date, i) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex-shrink-0 w-14 h-14 rounded-2xl border transition-all flex flex-col items-center justify-center gap-0.5 snap-start ${
                           ...
                           <div className="grid grid-cols-4 gap-1.5">
                              {timeSlots.map((slot, i) => (
                                <button
                                  ...
                                  className={`relative py-2.5 rounded-xl text-[9px] font-black transition-all flex flex-col items-center gap-1 ${
                            ...
                            {MODE_OPTIONS.map((mode) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`py-2.5 rounded-2xl border text-[9px] font-black uppercase tracking-widest transition-all space-y-1 ${
                      ...
                      <div className="pt-2">
                        <button
                          ...
                          className="w-full py-3 rounded-[1.25rem] bg-primary-600 text-white text-[11px] font-black uppercase tracking-[0.18em] shadow-xl shadow-primary-600/20 hover:bg-primary-700 transition-all active:scale-95 disabled:bg-slate-100 disabled:text-slate-300 flex items-center justify-center gap-2.5"
                        >
                          {offerSending ? (
                            <Loader2 className="animate-spin" size={15} />
                          ) : (
                            <>FIX SELECTION & SEND OFFER <Check size={13} strokeWidth={3} /></>
                          )}
                        </button>
                        <p className="text-[8px] font-bold text-slate-400 text-center mt-2.5 uppercase tracking-widest leading-relaxed">
